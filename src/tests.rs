...
mod periodic_tests {
    ...
    fn test_gaussian_kernel_symmetry() {
        ...
        assert!(kernel.iter().all(|&v| v >= 0.0), "Kernel values should be non-negative");
...
mod tensor_tests {
    ...
    fn test_qkv_projection_shapes() {
        ...
        let q_heads = q.reshape((batch, seq_len, n_heads, head_dim)).unwrap()
            .transpose(1, 2).unwrap();
        let k_heads = k.reshape((batch, seq_len, n_heads, head_dim)).unwrap()
            .transpose(1, 2).unwrap();
        let v_heads = v.reshape((batch, seq_len, n_heads, head_dim)).unwrap()
            .transpose(1, 2).unwrap();
...
mod training_tests {
    use crate::training::{
        TrainingConfig, LRScheduler, TrainingMetrics, 
        mse_loss, generate_random_batch, LossType, TorusLoss
    };
    ...
    fn test_generate_random_batch_various_sizes() {
        ...
        for (batch, seq, d_model, vocab) in [(1, 8, 16, 50), (8, 32, 64, 100), (16, 64, 128, 1000)] {
            let (inputs, targets) = generate_random_batch(batch, seq, d_model, vocab, &device).unwrap();
    ...
    fn test_torus_loss_creation() {
        ...
        let loss_fn_with_smoothing = TorusLoss::new(LossType::CrossEntropy)
            .with_label_smoothing(0.1);
        let loss_fn_geodesic = TorusLoss::new(LossType::GeodesicAware)
            .with_geodesic_weight(0.1);
...
mod edge_case_tests {
    ...
    fn test_torus_aspect_ratios() {
        ...
        assert!(thin_torus.major_radius / thin_torus.minor_radius > 
    ...
    fn test_wrap_idempotent() {
        ...
        for i in -20..20_i64 {
            for j in -20..20_i64 {
                ...
                let (rewrapped_i, rewrapped_j) = boundary.wrap_2d(wrapped_i as i64, wrapped_j as i64);
