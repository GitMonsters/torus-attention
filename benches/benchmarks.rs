...
use torus_attention::{
    BidirectionalTorusConfig, PeriodicBoundary, TorusCoordinate, TorusDistanceMatrix, TorusManifold,
};
...
fn bench_geodesic_distance(c: &mut Criterion) {
    ...
    c.bench_function("geodesic_distance", |b| {
        b.iter(|| c1.geodesic_distance(black_box(&c2)))
    });
}
...
fn bench_cartesian_conversion(c: &mut Criterion) {
    ...
    c.bench_function("cartesian_conversion", |b| {
        b.iter(|| coord.to_cartesian(black_box(2.0), black_box(1.0)))
    });
}
...
fn bench_grid_generation(c: &mut Criterion) {
    ...
    for size in [8, 16, 32, 64].iter() {
        ...
        group.bench_with_input(BenchmarkId::from_parameter(size), size, |b, &size| {
            b.iter(|| torus.generate_grid(black_box(size), black_box(size)))
        });
    }
    group.finish();
}
...
fn bench_distance_matrix(c: &mut Criterion) {
    ...
    for size in [8, 16, 32].iter() {
        ...
        group.bench_with_input(BenchmarkId::from_parameter(size), &coords, |b, coords| {
            b.iter(|| TorusDistanceMatrix::from_coordinates(black_box(coords)))
        });
    }
    group.finish();
}
...
fn bench_wrap_2d(c: &mut Criterion) {
    ...
    c.bench_function("wrap_2d", |b| {
        b.iter(|| boundary.wrap_2d(black_box(100), black_box(-50)))
    });
}
...
fn bench_gaussian_kernel(c: &mut Criterion) {
    ...
    for radius in [3, 5, 7, 9].iter() {
        group.bench_with_input(BenchmarkId::from_parameter(radius), radius, |b, &radius| {
            b.iter(|| boundary.gaussian_kernel(black_box(1.0), black_box(radius)))
        });
    }
    group.finish();
}
...
fn bench_tensor_creation(c: &mut Criterion) {
    ...
    for (batch, seq, dim) in [(1, 32, 64), (4, 64, 128), (8, 128, 256)].iter() {
        ...
        group.bench_function(&name, |b| {
            b.iter(|| Tensor::randn(0.0f32, 1.0, (*batch, *seq, *dim), &device).unwrap())
        });
    }
    group.finish();
}
...
fn bench_matmul(c: &mut Criterion) {
    ...
    for size in [32, 64, 128, 256].iter() {
        ...
        group.bench_with_input(
            ...
            &(a.clone(), b.clone()),
            |bench, (a, b)| bench.iter(|| a.matmul(black_box(b)).unwrap()),
        );
    }
    group.finish();
}
...
fn bench_attention_scores(c: &mut Criterion) {
    ...
    for seq_len in [32, 64, 128, 256].iter() {
        ...
        group.throughput(Throughput::Elements(
            (batch * n_heads * seq_len * seq_len) as u64,
        ));
        group.bench_with_input(
            BenchmarkId::from_parameter(seq_len),
...
fn bench_stream_combination(c: &mut Criterion) {
    ...
    for seq_len in [64, 128, 256].iter() {
        ...
        group.throughput(Throughput::Elements(
            (batch * seq_len * d_model * n_streams) as u64,
        ));
        group.bench_with_input(
            BenchmarkId::from_parameter(seq_len),
            ...
            |b, (streams, weights_vec)| {
                b.iter(|| {
                    let mut combined =
                        Tensor::zeros((batch, *seq_len, d_model), DType::F32, &device).unwrap();
...
fn bench_config_deserialization(c: &mut Criterion) {
    ...
    c.bench_function("config_from_json", |b| {
        b.iter(|| serde_json::from_str::<BidirectionalTorusConfig>(black_box(&json)).unwrap())
    });
}
...
criterion_group!(periodic_benches, bench_wrap_2d, bench_gaussian_kernel,);
...
criterion_group!(stream_benches, bench_stream_combination,);
...
criterion_group!(compounding_benches, bench_ema_computation,);
