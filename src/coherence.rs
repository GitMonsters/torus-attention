...
impl SenseOfCoherence {
    ...
    pub fn score(&self) -> f64 {
        ...
        0.25 * self.comprehensibility + 0.25 * self.manageability + 0.50 * self.meaningfulness
    }
...
impl SharedMentalModel {
    ...
    pub fn update_from_attention(
        ...
        for i in 0..self.n_streams {
            for j in (i + 1)..self.n_streams {
                ...
                // EMA update
                self.alignment_matrix[i][j] = (1.0 - learning_rate) * self.alignment_matrix[i][j]
    ...
    pub fn alignment_weights(&self) -> Vec<f64> {
        ...
        for i in 0..self.n_streams {
            let avg_alignment: f64 =
                self.alignment_matrix[i].iter().sum::<f64>() / self.n_streams as f64;
...
impl CognitiveCoherenceLayer {
    ...
    pub fn update_smm(&mut self, stream_attentions: &[Tensor]) -> TorusResult<()> {
        self.smm
            .update_from_attention(stream_attentions, self.config.smm_learning_rate)
    }
    ...
    pub fn coherent_compound(&mut self, input: &Tensor, attention: &Tensor) -> TorusResult<Tensor> {
