...
fn demo_periodic_boundary() {
    ...
    println!("  Grid spacing: du={:.4}, dv={:.4}", boundary.du, boundary.dv);
...
fn demo_vortex_dynamics() {
    ...
    println!("  Final circulation:   {:.4} (conserved)", dynamics.total_circulation());
...
fn demo_bidirectional_flow() {
    ...
    println!("  Forward:  {} (sign={})", 
             if FlowDirection::Forward.is_forward() { "→" } else { "←" },
             FlowDirection::Forward.sign());
    println!("  Backward: {} (sign={})",
             if FlowDirection::Backward.is_forward() { "→" } else { "←" },
...
fn demo_parallel_streams() {
    ...
    for id in StreamId::all() {
        let dir = if id.is_forward() { "Forward " } else { "Backward" };
        let pair = id.pair();
        println!("│ {:16} │ {:11} │ {:10} │", 
                 id.name(), dir, pair.name());
    }
    println!("└──────────────────┴─────────────┴────────────┘");
    ...
    println!("  Spiral winding:  {:.6} (golden ratio)", config.spiral_winding);
...
fn demo_compounding() {
    ...
    println!("  α range:          [{}, {}]", config.min_alpha, config.max_alpha);
...
fn demo_full_system() {
    ...
    println!("├─────────────────────────────────────────────────────────────────────┤");
    println!("│  Model dimension:     {:4}                                          │", config.d_model);
    println!("│  Feed-forward dim:    {:4}                                          │", config.d_ff);
    println!("│  Attention heads:     {:4}                                          │", config.n_heads);
    println!("│  Number of layers:    {:4}                                          │", config.n_layers);
    println!("│  Sequence length:     {:4} ({}×{})                               │", 
             config.seq_len(), config.n_major, config.n_minor);
    println!("├─────────────────────────────────────────────────────────────────────┤");
    println!("│ TORUS GEOMETRY                                                      │");
    println!("├─────────────────────────────────────────────────────────────────────┤");
    println!("│  Major radius:        {:4.1}                                          │", config.major_radius);
    println!("│  Minor radius:        {:4.1}                                          │", config.minor_radius);
    println!("│  Geodesic σ:          {:4.1}                                          │", config.geodesic_sigma);
    println!("├─────────────────────────────────────────────────────────────────────┤");
    println!("│ BIDIRECTIONAL PROCESSING                                            │");
    println!("├─────────────────────────────────────────────────────────────────────┤");
    println!("│  8-stream parallel:   {:5}                                         │", config.use_parallel_streams);
    println!("│  Spiral winding (φ):  {:6.4}                                       │", config.spiral_winding);
    println!("│  Parallel execution:  {:5}                                         │", config.parallel_execution);
    println!("├─────────────────────────────────────────────────────────────────────┤");
    println!("│ COMPOUNDING                                                         │");
    println!("├─────────────────────────────────────────────────────────────────────┤");
    println!("│  EMA compounding:     {:5}                                         │", config.use_compounding);
    println!("│  Base α:              {:5.2}                                         │", config.ema_alpha);
    println!("│  Learnable α:         {:5}                                         │", config.learnable_alpha);
    println!("│  Use momentum:        {:5}                                         │", config.use_momentum);
    println!("└─────────────────────────────────────────────────────────────────────┘");
    ...
    println!("  Total:       {} ({:.1}M)", total_params, total_params as f64 / 1_000_000.0);
}
